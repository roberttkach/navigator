...
class Trimmer:
    ...
    async def execute(self, count: int = 1) -> None:
        if count <= 0:
            channel.emit(logging.INFO, LogCode.RENDER_SKIP, op="pop", note="count_le_0")
            return
        history = await self._ledger.recall()
        channel.emit(logging.DEBUG, LogCode.HISTORY_LOAD, op="pop", history={"len": len(history)})
        if len(history) <= 1:
            return
        ...
        channel.emit(logging.DEBUG, LogCode.HISTORY_SAVE, op="pop", history={"len": len(trimmed)})
